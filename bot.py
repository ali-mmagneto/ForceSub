...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        if x is True:
            ...
            butt = [Button.url("Kanalım", url=f"https://t.me/{channel}")]
        else:
            ...
            butt = [Button.url("Kanalım", url=f"https://t.me/{channel}"), Button.inline("Sesimi Aç", data=f"unmute_{user.id}")]
...
async def mute_on_msg(event):
    ...
    if x is False:
        ...
        try:
            ...
            print(str(e))
            return
        await event.reply(f"Merhaba {nm}, görünüşe göre kanalıma katılmamışsın. Lütfen Kanalıma (@{channel}) katıl ve butona basarak sesini aç!", buttons=[[Button.url("Kanalım", url=f"https://t.me/{channel}")], [Button.inline("Sesimi Aç", data=f"unmute_{event.sender_id}")]])
...
async def _(event):
    ...
    if uid == event.sender_id:
        ...
        if x is False:
            await event.answer(f"Hala Kanalıma (@{channel}) katılmamışsın!", cache_time=0, alert=True)
        elif x is True:
            try:
                ...
                print(str(e))
                return
            msg = f"Merhaba {(await event.get_chat()).title}, {nm}!\nSeni görmek güzeldi!"
            butt = [Button.url("Kanalım", url=f"https://t.me/{channel}")]
            await event.edit(msg, buttons=butt)
    else:
        await event.answer("Sen Zaten bizim sadık üyemizsin! Bu snein için değil!", cache_time=0, alert=True)

@BotzHub.on(events.NewMessage(pattern="/start"))
async def strt(event):
    await event.reply(f"Merhaba. Ben @{channel} için yapılan Forcesub Botuyum!", buttons=[Button.url("Kanalım", url=f"https://t.me/{channel}")])
